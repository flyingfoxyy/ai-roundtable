...
**声明之外还要能核对**：从第二版起，评审 prompt 会附上 v(n-1)→v(n) 的 unified diff，
用来核对主编声称的处置与实际改动是否一致——声称"采纳"却没有对应改动、或悄悄回退已解决的内容，
都应据实 BLOCK。

**打转即止损**：连续两版硬伤数未下降、且期间出现过重提时，会议提前散会并说明理由，
不再耗完剩余轮数预算（两个条件缺一不可——硬伤持平但每轮都是新问题，说明讨论在深挖而非打转）。
此时最合理的下一步是做实验，然后用 `--continue` 带着新证据回来。
